...
public class JoinTest {
    ...
    public void test1() throws Exception {
        ...
        List<Team> result = queryFactory
                .select(QTeam.team)
                .from(QTeam.team)
                .leftJoin(QTeam.team.members, QMember.member)
                .where(QTeam.team.rank.loe(2))
                .fetch();
    ...
    public void test5() throws Exception {
        ...
        List<Tuple> result = queryFactory
                .select(QTeam.team, QMember.member)
                .from(QTeam.team)
                .leftJoin(QTeam.team.members, QMember.member)
                .on(QMember.member.age.goe(20))
                .where(QTeam.team.rank.loe(2))
        ...
        System.out.println(result.get(0).get(QMember.member));
        System.out.println(result.get(0).get(0, Member.class));
        System.out.println("======");
        System.out.println();

        Assertions.assertTrue(result.get(0)
                .get(QTeam.team).getMembers().size() == 3); // team의 members에 3명의 팀원 포함됨 -> 원하는 결과 X
    }
